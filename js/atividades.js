/* ========================================================
   CONEXÃO DIRETA COM O PAINEL: MANUTENÇÃO E AVISOS GLOBAIS
   ======================================================== */
(function verificarOrdensPainel() {
    const nomePagina = window.location.pathname.split("/").pop() || "atividades.html";
    const manutencao = JSON.parse(localStorage.getItem("sistema_manutencao") || "{}");

    // 1. Bloqueio por Manutenção (Geral ou Específica de Atividades)
    if (manutencao.geral === true || manutencao.atividades === true) {
        window.addEventListener("DOMContentLoaded", () => {
            const textoMsg = manutencao.textoAviso || "Estamos preparando novidades incríveis! Esta seção volta em instantes.";
            document.body.innerHTML = `
                <div style="min-height: 100vh; background-color: #0b1329; color: #f8fafc; display: flex; flex-direction: column; align-items: center; justify-content: center; text-align: center; padding: 25px; box-sizing: border-box; font-family: 'Segoe UI', sans-serif;">
                    ...
                    <div style="display: flex; gap: 12px; flex-wrap: wrap; justify-content: center;">
                        ...
                        <button onclick="window.location.reload()" style="background-color: #1e293b; color: #38bdf8; border: 1px solid #334155; padding: 10px 22px; border-radius: 20px; font-weight: bold; cursor: pointer; font-size: 0.95rem;">Tentar Novamente</button>
                    </div>
                </div>
            `;
        });
    ...
    // 2. Barra de Aviso Global no Topo
    ...
    if (configAviso.ativo && configAviso.texto && configAviso.texto.trim() !== "") {
        window.addEventListener("DOMContentLoaded", () => {
            ...
            barra.id = "barra-aviso-topo-portal";
            barra.style.cssText = `
                width: 100%;
            ...
            barra.innerHTML = `
                ...
                <button onclick="this.parentElement.remove()" style="background: transparent; border: none; color: #fff; font-size: 1.1rem; cursor: pointer; line-height: 1; padding: 0 4px; margin-left: 10px;">✕</button>
            `;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // ========================================================
    const bancoAtividades = (typeof DB !== "undefined" && DB.getAtividades) ? DB.getAtividades() : [];
    ...
    if (btnEnviarTarefa) {
        btnEnviarTarefa.addEventListener("click", () => {
            ...
            if (typeof DB !== "undefined" && DB.salvarSubmissao) {
    ...
    function carregarBoletim(nomeFiltro) {
        ...
        const todas = (typeof DB !== "undefined" && DB.getSubmissoes) ? DB.getSubmissoes() : [];
